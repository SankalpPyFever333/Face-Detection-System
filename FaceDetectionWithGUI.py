...
def TakeImages():
    ...
    if exists:
        with open("YourInfo/YourInfoDetails.csv", "r") as csvFile1:
            ...
            for row in reader:
                serial += 1
            serial = (serial//2) # doing this bcoz everytime when we enter a record in csv file, it gets entered by leaving one line everytime.
            csvFile1.close()
    else:
    ...
    if name.isalpha() or ' ' in name:
        ...
        while(True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite("TrainingAlgoImages\\"+name+"."+Id+"."+str(SampleNum)+".jpg", gray[y:y + h, x:x + w])
                # print("TrainingAlgoImages\\"+name +"."+Id+"."+str(SampleNum)+".jpg")
...
def TrackImage():
    check_haarcascadefile()
    checkFileExistence("YourInfo/")
    i = 0 #used for accessing the data from a csv file.
    recognizer = cv2.face.LBPHFaceRecognizer_create()
    ...
    if exist3:
        ...
        mess._show(title="Data Missing",message="click save profile to reset data")
        return
    harcascadePath = "Face.xml"
...
frame2 = tk.Frame(window, bg="#55aeff")
frame2.place(relx=0.51, rely=0.17, relwidth=0.38, relheight=0.80)

message3 = tk.Label(window, text="Face Recognition And Detection System",
                    fg="white", bg="#262375", width=55, height=1, font=('times', 29, ' bold '))
